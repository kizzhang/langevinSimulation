# -*- coding: utf-8 -*-
"""
Created on Sun Dec  5 13:17:52 2021
...
def Confined_abp_move(t,dt,N,r_b):
    for i in range(0, int(t/dt)-1): # time evolution
        ...
        # Run interactions
        for p1 in range(N):
            r_dx_new_p1, r_dy_new_p1, dphi_new_p1 = 0,0,0   # initializing increment for r_x, r_y, phi for i'th particle
            
            radial_vec = [r_x[p1,i],r_y[p1,i]] / np.sqrt((r_x[p1,i]**2+r_y[p1,i]**2))
            dphi_new_p1 =  v * np.sin(Angle_btw_2_vecs([np.cos(phi[p1,i]), np.sin(phi[p1,i])],-1*radial_vec))\
                * dt * 1/np.sqrt(r_x[p1,0]**2+r_y[p1,0]**2)
            print(np.sin(Angle_btw_2_vecs([np.cos(phi[p1,i]), np.sin(phi[p1,i])],-1*radial_vec)))
...
dt = 0.001     # time step between recorded positions
N = 1 #2000          # number of cells 

mu_plus = 0.3  #4#  # alignment strength
mu_minus = 0.1  #0.004   # anti-alignment strength

# Boundary box
box_r = 100      # boundary circle radius
...
for  i in range(0,1000):
    ...
    ax = plt.gca()
    plt.axis('off')
    plt.savefig(r"C:\Users\Kaneki\Desktop\circle\snap_{0}.png".format(i))
    plt.close()             
'''
