...
def ABP_move(t, dt, N, l): 
    for i in range(0, int(t/dt)-1): # time evolution
        ...
        for p1 in range(N):
            ...
            dx_p1_new = v * np.cos(theta[p1,i]) * dt #+ np.sqrt(2*Dt*dt) * np.random.randn()
            dy_p1_new = v * np.sin(theta[p1,i]) * dt #+ np.sqrt(2*Dt*dt) * np.random.randn()
            vx[p1,i] = v * np.cos(theta[p1,i])
            vy[p1,i] =  v * np.sin(theta[p1,i])
            for p2 in range(N):
                if p1 == p2:
                    continue
                #theta_p2_new = 0
                ...
                if r < zeta_ex:
                    ...
                    #dx_p2_new = dx_p2_new - k_r * (x[p1,i] - x[p2,i]) * dt
                    #dy_p2_new = dy_p2_new - k_r * (y[p1,i] - y[p2,i]) * dt 
                    
                # Angular exclusion
                if r <= epsilon_ex: # Exclusion region
                    alpha_ji = Angle([x[p2,i]-x[p1,i], y[p2,i]-y[p1,i]])
                    #alpha_ij = Angle([x[p1,i]-x[p2,i], y[p1,i]-y[p2,i]])
                    
                    theta_p1_new = theta_p1_new - k_rad * np.sin(alpha_ji-theta[p1,i]) * dt
                    #theta_p2_new = theta_p2_new - kappa * np.sin(alpha_ij-theta[p2,i]) * dt
              
                # Angular Alignment
                if r <= epsilon_aa: # Alignment region
                    alig_incre_p1 = mu_plus * (1-(r/epsilon_aa)**2) * dt * np.sin(theta[p2,i] - theta[p1,i])
                    #alig_incre_p2 = mu_plus * (1-(r/epsilon_aa)**2) * dt * np.sin(theta[p1,i] - theta[p2,i])
                    theta_p1_new = theta_p1_new + alig_incre_p1
                    #theta_p2_new = theta_p2_new + alig_incre_p2
                ...
                if r > epsilon_aa: # Anti-alignment region
                    ...
                    #anti_incre_p2 = mu_minus * 4 * (r-epsilon_aa)*(epsilon_int-r)/ (epsilon_int - epsilon_aa)**2 * dt* np.sin(theta[p1,i] - theta[p2,i])
                    theta_p1_new = theta_p1_new - anti_incre_p1
                    #theta_p2_new = theta_p2_new - anti_incre_p2

                # Add total angular increament of all potentials from p1 to p2  
                #theta[p2,i+1] = theta[p2,i+1] + theta_p2_new
                # Add total positional increament of exclusional potential from p1 to p2
                #x[p2, i+1] = x[p2, i+1] + dx_p2_new
                #y[p2, i+1] = y[p2, i+1] + dy_p2_new
